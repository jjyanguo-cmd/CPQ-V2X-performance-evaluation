...
def get_part_ms(name, N, part):
    ...
    if part == "rsu":
        if name.startswith("Ours"):
            ...
            res_us = r * N
        return res_us / 1000.0
    if part == "ta":
        return t * N / 1000.0
    raise ValueError("Unknown part")

def get_total_ms(name, N):
    return (
        get_part_ms(name, N, "vehicle")
        + get_part_ms(name, N, "rsu")
        + get_part_ms(name, N, "ta")
    )
...
for name in schemes:
    y = np.array([get_total_ms(name, int(N)) for N in N_vals])
    curve_cache[name] = y
    ax.plot(
        N_vals, y,
...
ax.set_ylabel(r'Total overhead (ms)', fontsize=AXLABEL_FS, labelpad=1.5)
ax.set_xlim(1, 1000)
...
axins = inset_axes(
    ...
    loc="center",
    borderpad=0.04,
    bbox_to_anchor=(0.35, 0.15, 0.40, 0.40),
...
out_pdf = OUTPUT_DIR / "Comp_Total.pdf"
out_png = OUTPUT_DIR / "Comp_Total.png"
